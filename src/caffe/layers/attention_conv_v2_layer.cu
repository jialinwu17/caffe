...
void AttentionConvolutionV2Layer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int n = 0; n < this->num_; ++n) {
      ...
      caffe_gpu_add( kernel_dim_ * height_ * width_,attention_col_buffer_.gpu_data(),att_input, attention_joint_col_buffer_.mutable_gpu_data());
      caffe_gpu_add_scalar( kernel_dim_ * height_ * width_,Dtype(-1.0), attention_joint_col_buffer_.mutable_gpu_data());
      const Dtype* att_col_buff = attention_joint_col_buffer_.gpu_data();
...
template <typename Dtype>
void AttentionConvolutionV2Layer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[0] || propagate_down[0]) {
      for (int n = 0; n < this->num_; ++n) {
        ...
        caffe_gpu_add(kernel_dim_ * height_ * width_,bottom_att_diff + n * kernel_dim_ * height_ * width_, attention_col_diff_buff_.gpu_data(),bottom_att_diff + n * kernel_dim_ * height_ * width_);
        }
      }
    }
}

INSTANTIATE_LAYER_GPU_FUNCS(AttentionConvolutionV2Layer);
