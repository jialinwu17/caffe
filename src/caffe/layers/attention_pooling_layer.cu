...
namespace caffe {
  ...
  __global__ void AttentionPoolingForward(
    const int nthreads,
    ...
    CUDA_KERNEL_LOOP(index, nthreads) {
      ...
      top_data[index] = top_data[index] /K;
    }
  }
  ...
  void AttentionPoolingLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    AttentionPoolingForward<Dtype> << <CAFFE_GET_BLOCKS(count),
  ...
  template <typename Dtype>
  __global__ void AttentionPoolingBackwardAtomic(
    const int nthreads,
    ...
    CUDA_KERNEL_LOOP(index, nthreads) {
      ...
      for (int m =0 ; m < output_dim; m++){
        ...
        bottom_diff_data[data_index] = top_diff[n*output_dim*K + m*K + k]*bottom_mask[mask_index]/K;

      }

      bottom_diff_mask[mask_index]  = bottom_diff_mask[mask_index] /K;
  ...
  void AttentionPoolingLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    ...
    AttentionPoolingBackwardAtomic<Dtype> << <CAFFE_GET_BLOCKS(count),
