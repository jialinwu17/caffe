    ...
    for (int i = 0; i < kernel_h; ++i) {
      for (int j = 0; j < kernel_w; ++j) {
        ...
        if ((b_h_ind + dilation_h * (i - kernel_h + 1 + h_pad ) < height) &&b_h_ind + dilation_h * (i - kernel_h + h_pad + 1) >=0){
          if ((b_w_ind + dilation_w * (i - kernel_w + w_pad + 1) < width) &&b_w_ind + dilation_w * (i - kernel_w  + w_pad + 1) >=0){
...
__global__ void object_forward(const int n, const Dtype* offset_col_buffer, const Dtype* weight, const int channels, const int num_output, const int height, const int width, const int kernel_h, const int kernel_w,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    const int weight_offset = (n_ind * width * height + h_ind * width + w_ind) * num_output * kernel_h * kernel_w;
    const int top_data_offset = ((n_ind * width * height + h_ind * width + w_ind) * kernel_h * kernel_w + h_s_ind * w_samples + w_s_ind)* num_output ;
...
template <typename Dtype>
void ObjectKernelConvolutionLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void ObjectKernelConvolutionLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(ObjectKernelConvolutionLayer);
