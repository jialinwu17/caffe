...
#include "caffe/layers/lilac_pooling_layer.hpp"
...
namespace caffe {
  ...
  __global__ void LilacPoolingForward(
    const int nthreads,
    ...
    CUDA_KERNEL_LOOP(index, nthreads) {
      ...
      int pw = index % (pooled_width + 2) - 1;
      int ph = (index /(pooled_width + 2) ) % (pooled_height+2) - 1;
      int ctop = (index / (pooled_width + 2) / (pooled_height+2)) % output_dim;
      int n = index / (pooled_width + 2) / (pooled_height+2)/ output_dim;
      ...
      int gw =min( max(pw,0), pooled_width);
      int gh = min( max(ph,0), pooled_height);
  ...
  template <typename Dtype>
  void LilacPoolingLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    LilacPoolingForward<Dtype> << <CAFFE_GET_BLOCKS(count),
  ...
  template <typename Dtype>
  __global__ void LilacPoolingBackwardAtomic(
    const int nthreads,
  ...
  template <typename Dtype>
  void LilacPoolingLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    ...
    // NOLINT_NEXT_LINE(whitespace/operators)
    LilacPoolingBackwardAtomic<Dtype> << <CAFFE_GET_BLOCKS(count),
  ...
  INSTANTIATE_LAYER_GPU_FUNCS(LilacPoolingLayer);
