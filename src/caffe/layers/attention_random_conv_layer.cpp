...
void AttentionRandomConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
 ...
 // printf("lilac !!\n");
  ...
  //#printf("lilac !!\n");
  mini = conv_param.mini();
    maxi = conv_param.maxi();
    inter_kernel = conv_param.inter_kernel();
}

template <typename Dtype>
void AttentionRandomConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ConvolutionParameter conv_param = this->layer_param_.convolution_param();
  const int first_spatial_axis = channel_axis_ + 1;
 ...
 // printf("kernel_dim_ : %d,output_shape_0: %d,output_shape_1: %d \n",kernel_dim_ * group_,output_shape_[0],output_shape_[1]);
  ...
  random_.Reshape(col_buffer_shape_);
...
template <typename Dtype>
void AttentionRandomConvolutionLayer<Dtype>::compute_output_shape() {
...
template <typename Dtype>
void AttentionRandomConvolutionLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void AttentionRandomConvolutionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
template <typename Dtype>
void AttentionRandomConvolutionLayer<Dtype>::forward_gpu_bias(Dtype* output,
    const Dtype* bias) {
...
template <typename Dtype>
void AttentionRandomConvolutionLayer<Dtype>::forward_cpu_bias(Dtype* output,
    const Dtype* bias) {
...
template <typename Dtype>
void AttentionRandomConvolutionLayer<Dtype>::backward_gpu_bias(Dtype* bias,
    const Dtype* input) {
  caffe_gpu_gemv<Dtype>(CblasNoTrans, num_output_, out_spatial_dim_, 1.,
      input, bias_multiplier_.gpu_data(), 1., bias);
}
template <typename Dtype>
void AttentionRandomConvolutionLayer<Dtype>::backward_cpu_bias(Dtype* bias,
    const Dtype* input) {
...
#ifdef CPU_ONLY
STUB_GPU(AttentionRandomConvolutionLayer);
#endif

INSTANTIATE_CLASS(AttentionRandomConvolutionLayer);
REGISTER_LAYER_CLASS(AttentionRandomConvolution);
