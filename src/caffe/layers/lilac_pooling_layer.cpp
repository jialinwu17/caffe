...
namespace caffe {
  ...
  void LilacPoolingLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  template <typename Dtype>
  void LilacPoolingLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
    ...
    top[0]->Reshape(
      bottom[1]->num(), output_dim_, pooled_height_ + 2, pooled_width_ + 2);
    mapping_channel_.Reshape(
      bottom[1]->num(), output_dim_, pooled_height_ + 2, pooled_width_ + 2);
  }

  template <typename Dtype>
  void LilacPoolingLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
    NOT_IMPLEMENTED;
  }

  template <typename Dtype>
  void LilacPoolingLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  INSTANTIATE_CLASS(LilacPoolingLayer);
  REGISTER_LAYER_CLASS(LilacPooling);
