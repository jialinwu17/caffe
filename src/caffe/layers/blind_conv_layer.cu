...
#include "caffe/layers/blind_conv_layer.hpp"
#include "caffe/util/math_functions.hpp"
namespace caffe {

template <typename Dtype>
void BlindConvolutionLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
    //  printf("reg_conv_forwarding\n");
  Dtype* weight = this->blobs_[0]->mutable_gpu_data();
  const Dtype* weight_mask = bottom[1]->gpu_data();
  const Dtype* bottom_data = bottom[0]->gpu_data();
  Dtype* masked_weight_data = masked_weight_.mutable_gpu_data();
 // printf("reg_conv_forwarding\n");
  Dtype* top_data = top[0]->mutable_gpu_data();
  caffe_gpu_mul( this->blobs_[0]->count(), weight, weight_mask, masked_weight_data);
  for (int n = 0; n < this->num_; ++n) {
    ...
    //  printf("reg_conv_forwarding\n");
      caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, conv_out_channels_ /*  N' */, conv_out_spatial_dim_/* H' * W' */ , kernel_dim_/* C * h * w */,
        (Dtype)1., masked_weight_data /*  C' * C * h * w  */, col_buffer_.gpu_data() /*  C * h * w * H' * W'   */, (Dtype)0.,  top_data + n * this->top_dim_); // C' * H' * W' 
     //   printf("reg_conv_forwarding\n");
      if (this->bias_term_) {
...
template <typename Dtype>
void BlindConvolutionLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  Dtype* mask_diff = bottom[1]->mutable_gpu_diff();
  Dtype* masked_weight_diff = masked_weight_.mutable_gpu_diff();
  ...
  if (this->param_propagate_down_[0] || propagate_down[0]) {
      for (int n = 0; n < this->num_; ++n) {
        ...
        if (this->param_propagate_down_[0]) {
          ...
          (Dtype)1., top_diff + n * this->top_dim_ , col_buff , (Dtype)0., masked_weight_diff);
          caffe_gpu_mul( this->blobs_[0]->count(),masked_weight_diff, weight, mask_diff); // only for bz == 1
          caffe_gpu_mul( this->blobs_[0]->count(),masked_weight_diff,bottom[1]->gpu_data(),weight_diff);
...
INSTANTIATE_LAYER_GPU_FUNCS(BlindConvolutionLayer);
